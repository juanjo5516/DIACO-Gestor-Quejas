...
@NamedQueries({
    @NamedQuery(
        ...
        query = "SELECT t FROM TipoImagenesQueja t WHERE t.habilitado = 'A' and t.id_queja = :id_queja and (t.id_flujo = 4) ORDER BY t.fecha_adicion"),
    @NamedQuery(
        name = "TipoImagenesQueja.findAllFlujoAtencionCon",
        query = "SELECT t FROM TipoImagenesQueja t WHERE t.habilitado = 'A' and t.id_queja = :id_queja and t.id_flujo in (1,2,3,4,6) and t.id_categoria_imagen in (1) ORDER BY t.fecha_adicion"),
    @NamedQuery(
        name = "TipoImagenesQueja.findZipFlujoConcVirt",
        query = "SELECT t FROM TipoImagenesQueja t WHERE t.habilitado = 'A' and t.id_queja = :id_queja and (t.id_flujo = 4 or t.id_categoria_imagen in (3) ) ORDER BY t.fecha_adicion"),
    @NamedQuery(
        name = "TipoImagenesQueja.findZipFlujoAtencionCon",
        query = "SELECT t FROM TipoImagenesQueja t WHERE t.habilitado = 'A' and t.id_queja = :id_queja and t.id_flujo in (1,2,3,4,6) and t.id_categoria_imagen in (1,2,3) ORDER BY t.fecha_adicion"),    
    @NamedQuery(
