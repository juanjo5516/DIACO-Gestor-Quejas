...
public class NumerosEnLetrasOrdinales {
    private int flag;
    public String num;
    ...
    public NumerosEnLetrasOrdinales(){
        /*constructor vacio*/
    }
    
    private String unidadOrdinal(int pNumero){
        switch (pNumero){
            case 9:
                num = "novena";
                break;
    ...
    public static String direccionAletras(String direccion) {
        direccion = direccion.replace(",", " , ").replace(".", " . ").replaceAll("\\s{2,10}", " ");
        NumerosEnLetrasOrdinales numLestrasOrd = new NumerosEnLetrasOrdinales();
        NumerosEnLetras numLetras = new NumerosEnLetras();
        String direccionLetras = "";
        String numeroLetras = "";
        String arrDir[] = direccion.split(" ");
        for (int i = 0; i < arrDir.length; i++) {
            String objString = arrDir[i];
            if (objString.toUpperCase().equals("CALLE") && i!=0) {
                if(NumerosEnLetrasOrdinales.isNumeric(arrDir[i - 1])){
                direccionLetras = direccionLetras +" "+ numLestrasOrd.convertir(Integer.parseInt(arrDir[i - 1])) + " calle";
                } else direccionLetras = direccionLetras+ " calle ";

            } else if (objString.toUpperCase().equals("AVENIDA")&& i!=0) {
                if(NumerosEnLetrasOrdinales.isNumeric(arrDir[i - 1])){
                direccionLetras = direccionLetras + " "+numLestrasOrd.convertir(Integer.parseInt(arrDir[i - 1])) + " avenida";
                } else direccionLetras = direccionLetras+ " avenida ";

            } else {
                int indexGuion = objString.indexOf("-");
                if (indexGuion != -1) {
                    if(NumerosEnLetrasOrdinales.isNumeric((objString.substring(0, indexGuion))) && NumerosEnLetrasOrdinales.isNumeric(objString.substring(indexGuion + 1)))
                    direccionLetras = direccionLetras + " " + numLetras.convertir(Integer.parseInt(objString.substring(0, indexGuion))) + " guion " + numLetras.convertir(Integer.parseInt(objString.substring(indexGuion + 1)));
                } else if (i <= arrDir.length - 2) {
                    if (NumerosEnLetrasOrdinales.isNumeric(objString) && !(arrDir[i + 1].toUpperCase().equals("CALLE") || arrDir[i + 1].toUpperCase().equals("AVENIDA"))) {
                        direccionLetras = direccionLetras + " " + numLetras.convertir(Integer.parseInt(objString));
                    } else if (!NumerosEnLetrasOrdinales.isNumeric(objString) && !objString.equals("")) {
                        direccionLetras = direccionLetras + " " + objString.toLowerCase();
                    }
                } else if (i == arrDir.length - 1 && NumerosEnLetrasOrdinales.isNumeric(objString)) {
                    direccionLetras = direccionLetras + " " + numLetras.convertir(Integer.parseInt(objString));
        ...
        direccionLetras = direccionLetras.replace(" , ", ", ").replace(" . ", ". ");
        return direccionLetras;
    }
