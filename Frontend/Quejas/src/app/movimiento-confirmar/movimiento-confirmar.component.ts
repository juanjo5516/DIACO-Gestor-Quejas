...
export class MovimientoConfirmarComponent implements OnInit {
  ...
  LoadCmbHacia(){
	...
	GetQuejaListPendienteAsignacion(){
		let tempstr='';
		this._mainTableService.getSimplePendAsig().subscribe((Data)=>{
			if(Data['reason'] == 'OK'){
				tempstr=Data['value'];
				console.log(tempstr);
  ...
  MoverHaciaUpdate(){
	...
	GuardarMovimiento(){
		...
		if(loc_str.length >0){
			...
			this._movimExpService.addData(loc_str, this.MoverHaciaCtrl.value, 1, 1 ).subscribe((retvalue)=>{
	...
	GuardarIndividual(){
		...
		this._movimExpService.addDataInd(this.HaciaIndCtrl.value, this.SedeCtrl.value, this.sing_queja['id_queja'] ).subscribe((retvalue)=>{
			console.log(retvalue);
			if(retvalue["reason"] == 'OK'){
				var tempstr=retvalue['value'];
				if(tempstr != null)	{
	...
	GetListaMovimientos(){
		this._movimExpService.getDataTop5().subscribe((retvalue)=>{
			if(retvalue["reason"] == 'OK'){
				var tempstr=retvalue['value'];
				if(tempstr != null)	{
