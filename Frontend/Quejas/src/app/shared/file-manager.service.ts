...
import { BASE_URL_REST_FILE } from '../atencion-consumidor-const'
...
export class FileManagerService {
  ...
  getPlantilla(id){
	  var localURL=this.baseUrl+'/plantilla/'+id+'/'+this._submitFormService.Get_token();
	  console.log(localURL);
  ...
  getCatFile(idqueja,idcategoria){
	  var localURL=this.baseUrl+'/interna/'+idqueja+'/'+idcategoria+'/'+this._submitFormService.Get_token();
	  console.log(localURL);
	  return this._http.get(localURL,this.httpOptions).pipe(map(this.extractData));  
  }
  
  getCatProveedor(idproveedor,idcategoria){
	  var localURL=this.baseUrl+'/proveedor/'+idproveedor+'/'+idcategoria+'/'+this._submitFormService.Get_token();
	  console.log(localURL);
	  return this._http.get(localURL,this.httpOptions).pipe(map(this.extractData));  
  }
  
  getProveedorAll(idproveedor){
	  var localURL=this.baseUrl+'/proveedorall/'+idproveedor+'/'+this._submitFormService.Get_token();
	  console.log(localURL);
	  return this._http.get(localURL,this.httpOptions).pipe(map(this.extractData));  
  }
  
  getImage(id){
	  var localURL=this.baseUrl+'/image/'+id+'/'+this._submitFormService.Get_token();
	  console.log(localURL);
	  return this._http.get(localURL, {responseType: 'blob'}
	  );  
  }
  ...
  getImageInterna(id){
	  var localURL=this.baseUrl+'/imageinterna/'+id+'/'+this._submitFormService.Get_token();
	  console.log(localURL);
	  return this._http.get(localURL, {responseType: 'blob'}
	  );  
  }

  getImageProveedor(id){
	  var localURL=this.baseUrl+'/imageproveedor/'+id+'/'+this._submitFormService.Get_token();
	  console.log(localURL);
	  return this._http.get(localURL, {responseType: 'blob'}
	  );  
  }
  ...
  postNewImage(filecontrol, idqueja, id_tipo_imagen, id_flujo, id_categoria_imagen){
		...
		formDataloc.append('id_flujo', id_flujo );
  ...
  postNewFile(filecontrol, idqueja, idcategoria, id_flujo){
		var localURL=this.baseUrl+'/interna';
		console.log(localURL);
		let formDataloc = new FormData();
        formDataloc.append('attachment', filecontrol );
		formDataloc.append('id_queja', idqueja );
		//5 docx
		formDataloc.append('id_tipo_imagen', '5' );
		formDataloc.append('id_categoria_imagen', idcategoria );
		formDataloc.append('id_flujo', id_flujo );
		formDataloc.append('token', this._submitFormService.Get_token() );
		const req =  new HttpRequest<any>('POST', localURL, formDataloc, {
			reportProgress: true
  ...
  postNewFileLinked(filecontrol, idqueja, idcategoria, id_flujo, registro, id_resfinal ){
		var localURL=this.baseUrl+'/internalinked';
		console.log(localURL);
		let formDataloc = new FormData();
        formDataloc.append('attachment', filecontrol );
		formDataloc.append('id_queja', idqueja );
		...
		formDataloc.append('id_flujo', id_flujo );
		formDataloc.append('token', this._submitFormService.Get_token() );
		formDataloc.append('registro', registro );
		formDataloc.append('id_resfinal', id_resfinal );
		console.log(formDataloc);
		const req =  new HttpRequest<any>('POST', localURL, formDataloc, {
			reportProgress: true
		});
		return this._http.request( req );
	...
	postNewScan(filecontrol, idqueja, idcategoria, id_tipo_imagen, id_flujo){
		var localURL=this.baseUrl+'/interna';
		console.log(localURL);
		let formDataloc = new FormData();
        formDataloc.append('attachment', filecontrol );
  ...
  postNewProveedor(filecontrol, idcategoria, idproveedor, id_tipo_imagen){
		var localURL=this.baseUrl+'/proveedor';
		console.log(localURL);
		let formDataloc = new FormData();
        formDataloc.append('attachment', filecontrol );
		formDataloc.append('id_proveedor', idproveedor );
		formDataloc.append('id_tipo_imagen', id_tipo_imagen );
		formDataloc.append('id_categoria_imagen', idcategoria );
		formDataloc.append('token', this._submitFormService.Get_token() );
		const req =  new HttpRequest<any>('POST', localURL, formDataloc, {
  ...
  downloadPNGFile(data) {
	...
	VerifyDOCX(tipo, nombre){
		if(tipo == 'application/vnd.openxmlformats-officedocument.wordprocessingml.document'){
			...
			//revisar tipo en base al nombre porque el tipo vienen en blanco
			var last4 = nombre.substr(nombre.length - 4);
			console.log('verificacion archivo 2:',nombre,last4);
			if(last4.toLowerCase() == 'docx'){
				return 5;
			}
		}
		return null;
