...
import {LSTFILTRO_SERVICIOSPUBLICOS, FrmMainParam, BaseCmbClass} from "../atencion-consumidor-const" 
...
@Component({
  selector: 'app-main-servicios-publicos',
  templateUrl: './main-servicios-publicos.component.html',
  styleUrls: ['./main-servicios-publicos.component.css'],
  animations: [
		trigger('EnterLeave', [
...
export class MainServiciosPublicosComponent implements OnInit {
  ...
  MtDialogCatProveedor : MatDialogRef<CatProveedorComponent>;
	...
	@ViewChild(TablaMainServPubComponent)
     private myChild: TablaMainServPubComponent;
  ...
  ngOnInit() {
	    ...
	    this.Acceso();
		this.Token();
		this.list_filtro=LSTFILTRO_SERVICIOSPUBLICOS;
	...
	Acceso(){
		...
		if(this._submitFormService.Get_Servicios_Publicos()>1)
			this.flagisAdminRevisor=true;
		
		if(this._submitFormService.Get_Servicios_Publicos()>2)
  ...
  OpenAccionPendiente(){
	...
	openBusqueda(){
		...
		dialogConfig.height = '100%';
		dialogConfig.width = '1000px';
		dialogConfig.data = {Usuario: this._submitFormService.Get_userid(), Rol: this._submitFormService.Get_Servicios_Publicos(), Flujo: 6};
