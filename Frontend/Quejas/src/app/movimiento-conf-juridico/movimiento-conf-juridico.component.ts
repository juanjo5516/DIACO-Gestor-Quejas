...
@Component({
  selector: 'app-movimiento-conf-juridico',
  templateUrl: './movimiento-conf-juridico.component.html',
  styleUrls: ['./movimiento-conf-juridico.component.css'],
  animations: [
		trigger('EnterLeave', [
...
export class MovimientoConfJuridicoComponent implements OnInit {
  ...
  constructor(@Inject(MAT_DIALOG_DATA) public data: any, private _sedeservice:SedeService, private _catalogoservice:CatalogoService, public dialogRef: MatDialogRef<MovimientoConfJuridicoComponent>, private _quejaService: QuejaService, private _registrosservice:RegistrosService, private _mainTableService:MainTableService, private _movimExpService:MovimExpService, private _compermservice:CompermanenteService) { 
		this.htmlQuejaNo='';
  ...
  LoadCmbHacia(){
		let tempstr='';
		this._catalogoservice.getDptoInternoMovHaciaJur().subscribe((Data)=>{
			if(Data['reason'] == 'OK'){
				tempstr=Data['value'];
				if(tempstr != null)	{
	...
	GetQuejaListSimpleVyV(){
		let tempstr='';
		this._mainTableService.getJurSimpleQuejaVerificacion().subscribe((Data)=>{
			if(Data['reason'] == 'OK'){
				tempstr=Data['value'];
	...
	GetQuejaListSimpleAtCon(){
		let tempstr='';
		this._mainTableService.getJurSimpleQuejaAtCon().subscribe((Data)=>{
			if(Data['reason'] == 'OK'){
				tempstr=Data['value'];
				this.lst_quejasimple=JSON.parse('['+tempstr.slice(0, -1) +']');
				if(tempstr != null && tempstr!='')	{
	...
	GetQuejaListSimpleArchivo(){
		let tempstr='';
		this._mainTableService.getJurSimpleQuejaArchivo().subscribe((Data)=>{
			if(Data['reason'] == 'OK'){
				tempstr=Data['value'];
				console.log(tempstr);
				this.lst_quejasimple=JSON.parse('['+tempstr.slice(0, -1) +']');
	...
	GetQuejaListPendienteAsignacion(){
		let tempstr='';
		this._mainTableService.getJurSimplePendAsig().subscribe((Data)=>{
			if(Data['reason'] == 'OK'){
				tempstr=Data['value'];
				console.log(tempstr);
  ...
  MoverHaciaUpdate(){
	...
	GuardarMovimiento(){
		...
		if(loc_str.length >0){
			...
			this._movimExpService.addData(loc_str, this.MoverHaciaCtrl.value, 2, 2 ).subscribe((retvalue)=>{
	...
	GuardarIndividual(){
		...
		this._movimExpService.addDataIndJur(this.HaciaIndCtrl.value, this.SedeCtrl.value, this.sing_queja['id_queja'] ).subscribe((retvalue)=>{
			console.log(retvalue);
			if(retvalue["reason"] == 'OK'){
				var tempstr=retvalue['value'];
				if(tempstr != null)	{
	...
	GetListaMovimientos(){
		this._movimExpService.getDataTop5Jur().subscribe((retvalue)=>{
			if(retvalue["reason"] == 'OK'){
				var tempstr=retvalue['value'];
				if(tempstr != null)	{
