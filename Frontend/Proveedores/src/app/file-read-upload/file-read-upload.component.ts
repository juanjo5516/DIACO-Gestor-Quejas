...
import { LSTFILTRO_RESOLUCION, BaseCmbClass, List_Images } from '../conc-virt-const';
...
/*  async getAsyncData() {
	...
	ReadAllFiles():JSZip{
		var jszip= new JSZip();;
		for (var me of this.ListImages) {
			this._fileManagerService.getImage(me['id_imagen_queja']).subscribe((Data)=>{
				jszip.file(me['descripcion_imagen_otros'], Data, {base64: true});
			},(error)=>{
				console.log(error);
				this.flagDBError=true;
				this.SetSecTimerInfoError();
		  ...
		  if( this._fileManagerService.VerifyFileSize(this.ELattachment.nativeElement.files.item(0).size) ){
			  ...
			  if(tipo_imagen){
				  console.log(this.ELattachment.nativeElement.files[0]);
					...
					this._fileManagerService.postNewImage(this.ELattachment.nativeElement.files.item(0),this.id_queja,tipo_imagen,1).subscribe(event => {
		  ...
		  if( this._fileManagerService.VerifyFileSize(this.filedpiCtrl.nativeElement.files.item(0).size) ){
			  ...
			  if(tipo_imagen){
				  console.log(this.filedpiCtrl.nativeElement.files[0]);
					...
					this._fileManagerService.postNewImage(this.filedpiCtrl.nativeElement.files.item(0),this.id_queja,tipo_imagen,2).subscribe(event => {
						if (event.type === HttpEventType.UploadProgress) {
		  ...
		  if( this._fileManagerService.VerifyFileSize(this.filefacCtrl.nativeElement.files.item(0).size) ){
			  ...
			  if(tipo_imagen){
				  console.log(this.filefacCtrl.nativeElement.files[0]);
					...
					this._fileManagerService.postNewImage(this.filefacCtrl.nativeElement.files.item(0),this.id_queja,tipo_imagen,3).subscribe(event => {
						if (event.type === HttpEventType.UploadProgress) {
  ...
  ReadImageList(){
	  let tempstr='';
		this._fileManagerService.getImageList(this.id_queja,4).subscribe((Data)=>{
			if(Data['reason'] == 'OK'){
				tempstr=Data['value'];
				if(tempstr != null)	{
...
/*	ColorSelect(){
	...
	*/
	DownloadZip() {
		this._fileManagerService.getZip(this.id_queja,4).subscribe((Data)=>{
