...
@Component({
  selector: 'app-movimiento-expediente',
  templateUrl: './movimiento-expediente.component.html',
  styleUrls: ['./movimiento-expediente.component.css'],
  animations: [
		trigger('EnterLeave', [
...
export class MovimientoExpedienteComponent implements OnInit {
  ...
  constructor(@Inject(MAT_DIALOG_DATA) public data: any, private _sedeservice:SedeService, private _catalogoservice:CatalogoService, public dialogRef: MatDialogRef<MovimientoExpedienteComponent>, private _quejaService: QuejaService, private _registrosservice:RegistrosService, private _mainTableService:MainTableService, private _movimExpService:MovimExpService, private _compermservice:CompermanenteService, private _finalizarService:FinalizarService) { 
		this.htmlQuejaNo='';
  ...
  ngOnDestroy() {
	  ...
	  this._quejaService.getDataQueja(this.data.NoQueja).subscribe((retvalue)=>{
			if(retvalue["reason"] == 'OK'){
				...
				if(tempstr != null)	{
					...
					if(this.lst_queja[0]['id_estado_queja']>=129 && this.lst_queja[0]['id_estado_queja']<=133)
	...
	GuardarMovimiento(){
		if(confirm("¿Está seguro que desea mover esta queja?")) {
			...
			this._movimExpService.addDataPre(this.data.NoQueja, this.MoverHaciaCtrl.value, this.observacionesCtrl.value ).subscribe((retvalue)=>{
