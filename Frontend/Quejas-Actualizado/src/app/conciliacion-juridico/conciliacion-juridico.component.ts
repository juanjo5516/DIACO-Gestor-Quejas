...
@Component({
  selector: 'app-conciliacion-juridico',
  templateUrl: './conciliacion-juridico.component.html',
  styleUrls: ['./conciliacion-juridico.component.css'],
  animations: [
		trigger('EnterLeave', [
...
export class ConciliacionJuridicoComponent implements OnInit {
  ...
  registrodata2;linkdescription2;idimagenactacon2;tipoactacon2;linkdescriptionCorr;
  lst_queja;flagformvisible;

  constructor(private _registrosservice:RegistrosService, private _fileManagerService:FileManagerService, @Inject(MAT_DIALOG_DATA) public data: any, public dialogRef: MatDialogRef<ConciliacionJuridicoComponent>, private _quejaService: QuejaService, private _seguridadService:SeguridadService) { 
	...
	this.linkdescriptionCorr='';
  }

  ngOnInit() {
	  this.GetLink(this.data.NoQueja);
	  this.GetLink2(this.data.NoQueja);
	  this.GetCorrelativo(this.data.NoQueja);
	  this.QuejaCheck();
	...
	ReadFile(){
      ...
      if(this.linkdescriptionCorr=='') {
		this._registrosservice.addGetActaConciliacionCorrelativo(this.data.NoQueja).subscribe((retvalue)=>{
			if(retvalue["reason"] == 'OK'){
				...
				if(tempstr != null)	{
					...
					this.linkdescriptionCorr='Correlativo: '+this.registrodata[0]['idCorrelativo'];
		...
		this._fileManagerService.getPlantilla(7).subscribe((Data)=>{
			console.log(Data);
  ...
  GetCorrelativo(idqueja){
	//12 acta de conciliacion juridico

	this._registrosservice.read_ActaConciliacionCorrelativo(this.data.NoQueja).subscribe((retvalue)=>{
		  if(retvalue["reason"] == 'OK'){
			  var tempstr=retvalue['value'];
  ...
  GetLink2(idqueja){
	  //13 acta de conciliacion juridico scan
 	  let idcategoria=13;
	  this._fileManagerService.getCatFile(idqueja, idcategoria).subscribe((retvalue)=>{
			if(retvalue["reason"] == 'OK'){
				var tempstr=retvalue['value'];
		  ...
		  if( this._fileManagerService.VerifyFileSizeInt(this.ELattachment.nativeElement.files.item(0).size) ){
			  ...
			  if(tipo_imagen){
				  console.log(this.ELattachment.nativeElement.files[0]);
					...
					//12 - acta de conciliacion
					this._fileManagerService.postNewFile(this.ELattachment.nativeElement.files.item(0),this.data.NoQueja, 12,2).subscribe(event => {
		  ...
		  if( this._fileManagerService.VerifyFileSize(this.ELattachment2.nativeElement.files.item(0).size) ){
			  ...
			  if(tipo_imagen){
				  console.log(this.ELattachment2.nativeElement.files[0]);
					...
					this._fileManagerService.postNewScan(this.ELattachment2.nativeElement.files.item(0),this.data.NoQueja,13,tipo_imagen,2).subscribe(event => {
						if (event.type === HttpEventType.UploadProgress) {
