...
import {LSTFILTRO_JURIDICO, FrmMainParam, BaseCmbClass} from "../atencion-consumidor-const" 
...
@Component({
  selector: 'app-main-juridico',
  templateUrl: './main-juridico.component.html',
  styleUrls: ['./main-juridico.component.css'],
  animations: [
		trigger('EnterLeave', [
...
export class MainJuridicoComponent implements OnInit {
list_filtro:BaseCmbClass[];
  ...
  MtDialogConfMov: MatDialogRef<MovimientoConfJuridicoComponent>;
  ...
  //tablechildflag:boolean;
	...
	@ViewChild(TablaMainJuridicoComponent)
     private myChild: TablaMainJuridicoComponent;
  ...
  ngOnInit() {
	...
	Acceso(){
		...
		if(this._submitFormService.Get_Juridico()>2)
  ...
  OpenAccionPendiente(){
	...
	openBusqueda(){
		...
		dialogConfig.height = '100%';
		dialogConfig.width = '1000px';
		dialogConfig.data = {Usuario: this._submitFormService.Get_userid(), Rol: this._submitFormService.Get_Atencion_Consumidor(), Flujo: 2};
	...
	openReasignar(){
		...
		dialogConfig.width = '1000px';
		dialogConfig.data = {Rol: this._submitFormService.Get_Atencion_Consumidor(), Flujo: 2};
