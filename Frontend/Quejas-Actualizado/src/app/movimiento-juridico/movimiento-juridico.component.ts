...
@Component({
  selector: 'app-movimiento-juridico',
  templateUrl: './movimiento-juridico.component.html',
  styleUrls: ['./movimiento-juridico.component.css'],
  animations: [
		trigger('EnterLeave', [
...
export class MovimientoJuridicoComponent implements OnInit {
lst_mhacia: BaseCmbClass[];
  ...
  constructor(@Inject(MAT_DIALOG_DATA) public data: any, private _sedeservice:SedeService, private _catalogoservice:CatalogoService, public dialogRef: MatDialogRef<MovimientoJuridicoComponent>, private _quejaService: QuejaService, private _registrosservice:RegistrosService, private _mainTableService:MainTableService, private _movimExpService:MovimExpService, private _compermservice:CompermanenteService, private _seguridadService:SeguridadService, private _finalizarService:FinalizarService) { 
		this.htmlQuejaNo='';
  ...
  ngOnDestroy() {
	  ...
	  this._quejaService.getDataQueja(this.data.NoQueja).subscribe((retvalue)=>{
			if(retvalue["reason"] == 'OK'){
				...
				if(tempstr != null)	{
					...
					if(this.lst_queja[0]['id_estado_queja']>=229 && this.lst_queja[0]['id_estado_queja']<=233)
	...
	GuardarMovimiento(){
		if(confirm("¿Está seguro que desea mover esta queja?")) {
			...
			this._movimExpService.addDataPreJur(this.data.NoQueja, this.MoverHaciaCtrl.value, this.observacionesCtrl.value ).subscribe((retvalue)=>{
