...
export class FinalizacionQuejaService {
  private baseUrl:string=BASE_URL_REST+'tipos-finalizacion';
	private httpOptions = {
  ...
  saveData (nombre,tipo,padre): Observable<any> {
    var LocalURL=this.baseUrl+'/save/'+nombre+'/'+tipo+'/'+padre+'/'+this._submitFormService.Get_username()+'/'+this._submitFormService.Get_token();
    console.log(LocalURL);
    return this._http.post(LocalURL,this.httpOptions).pipe(map(this.extractData));
  }  
  
  updateData (id,nombre,tipo,padre): Observable<any> {
    var LocalURL=this.baseUrl+'/update/'+id+'/'+nombre+'/'+tipo+'/'+padre+'/'+this._submitFormService.Get_username()+'/'+this._submitFormService.Get_token();
    console.log(LocalURL);
    return this._http.post(LocalURL,this.httpOptions).pipe(map(this.extractData));
  }  
