...
export class CatAreaService {
  private baseUrl:string=BASE_URL_REST+'tipos-area';
	private httpOptions = {
  ...
  saveData (nombre): Observable<any> {
    var LocalURL=this.baseUrl+'/save/'+nombre+'/'+this._submitFormService.Get_username()+'/'+this._submitFormService.Get_token();
    console.log(LocalURL);
    return this._http.post(LocalURL,this.httpOptions).pipe(map(this.extractData));
  }  
  
  updateData (id,nombre): Observable<any> {
    var LocalURL=this.baseUrl+'/update/'+id+'/'+nombre+'/'+this._submitFormService.Get_username()+'/'+this._submitFormService.Get_token();
    console.log(LocalURL);
    return this._http.post(LocalURL,this.httpOptions).pipe(map(this.extractData));
  }  
