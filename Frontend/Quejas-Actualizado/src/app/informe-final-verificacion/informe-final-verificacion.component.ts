...
@Component({
  selector: 'app-informe-final-verificacion',
  templateUrl: './informe-final-verificacion.component.html',
  styleUrls: ['./informe-final-verificacion.component.css'],
  animations: [
		trigger('EnterLeave', [
...
export class InformeFinalVerificacionComponent implements OnInit {
  ...
  constructor(private _registrosservice:RegistrosService, private _fileManagerService:FileManagerService, @Inject(MAT_DIALOG_DATA) public data: any, public dialogRef: MatDialogRef<InformeFinalVerificacionComponent>, private _quejaService: QuejaService, private _seguridadService:SeguridadService) { 
  ...
  ngOnInit() {
	...
	ReadFile(){
		...
		//4 informe final
		this._fileManagerService.getPlantilla(4).subscribe((Data)=>{
			console.log(Data);
		  ...
		  if( this._fileManagerService.VerifyFileSizeInt(this.ELattachment.nativeElement.files.item(0).size) ){
			  ...
			  if(tipo_imagen){
				  console.log(this.ELattachment.nativeElement.files[0]);
					...
					//10 informe final vyv
					this._fileManagerService.postNewFile(this.ELattachment.nativeElement.files.item(0),this.data.NoQueja, 10,2).subscribe(event => {
		  ...
		  if( this._fileManagerService.VerifyFileSize(this.ELattachment2.nativeElement.files.item(0).size) ){
			  ...
			  if(tipo_imagen){
				  console.log(this.ELattachment2.nativeElement.files[0]);
					...
					//11 informe final vyv scan
					this._fileManagerService.postNewScan(this.ELattachment2.nativeElement.files.item(0),this.data.NoQueja,11,tipo_imagen,2).subscribe(event => {
						if (event.type === HttpEventType.UploadProgress) {
